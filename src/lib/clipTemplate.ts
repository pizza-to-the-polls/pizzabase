...
export interface ClipRenderInput {
  ...
  reportedAt: string;
  /** Short-link slug — null means no short URL on the end-card. */
  shortUrlSlug: string | null;
  /**
   * Absolute path to the QR code PNG composited over the end-card, or null
   * for a text-only end-card (no slug, or QR generation failed). The PNG is
   * generated by the render lambda — this module stays pure and only
   * references the path in the args.
   */
 ...
 * deterministic derived set; shortUrlSlug is the clip's trackable slug.
 */
...
export function buildRenderPlan(input: ClipRenderInput): ClipRenderPlan {
  ...
  // When a short link exists and a QR PNG was generated for it, the QR is
  // composited over the card (third ffmpeg input, centered at y=980) and
  // the readable URL moves underneath it (y=1220). With no QR the URL stays
  // a text line at y=990; with no slug the card is brand + site text only.
  const hasQr = Boolean(input.shortUrlSlug && input.qrCodePath);
  ...
  if (input.shortUrlSlug) {
    ...
    const urlY = hasQr ? 1220 : 990;
    cardChain.push(
      `drawtext=textfile=${escapeFilterPath(urlPath)}:${fontSpec}` +
        `fontcolor=white:fontsize=36:x=(w-text_w)/2:y=${urlY}`,
    );
  }
  ...
  if (hasQr) {
    filterSegments.push("[2:v]format=yuv420p[qr]");
    filterSegments.push("[card][qr]overlay=(W-w)/2:980[cardwqr]");
  }
  filterSegments.push(
    `[base][${hasQr ? "cardwqr" : "card"}]overlay=0:0:enable='gte(t,${endCardStart})'[out]`,
  );
  ...
  if (hasQr) {
    // Still PNG input: overlay repeats its single frame (eof_action=repeat
    // is the default) across the whole enabled window at the tail.
    clipRenderArgs.push("-i", input.qrCodePath!);
  }
  clipRenderArgs.push(
    "-filter_complex",
    filterComplex,
